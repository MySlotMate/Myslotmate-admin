// Drag-and-drop file handling for upload zones.
// Ported from MySlotmate-Frontend/src/hooks/useDragDrop.ts.
      ...
      if (accept) {
        return fileArray.filter((file) => {
          ...
          if (accept.includes('*')) {
            ...
            return acceptTypes.some((type) => type === '*' || (ext && acceptTypes.includes(ext)));
          }
          return true;
        });
      }
