    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-4">
        ...
        <div className="flex flex-wrap gap-3">
          <Button variant="secondary" className="flex items-center gap-1.5" onClick={() => setIsNotifModalOpen(true)}>
            <Bell className="h-4 w-4 text-brand-600" /> WhatsApp Alerts
          </Button>
      ...
      <div className="rounded-3xl border border-brand-100/80 bg-white/95 shadow-soft backdrop-blur-md p-5">
        <div className="max-w-md">
          <div className="flex items-center gap-3 rounded-2xl border border-brand-100 bg-white px-4 py-3 shadow-sm">
            <svg className="h-4 w-4 text-slate-400" viewBox="0 0 24 24" fill="none" strokeWidth="1.9">
      ...
      {loading ? (
        ...
        <Table headers={['Host Name', 'City', 'Experiences', 'Bookings', 'Revenue', 'Status', 'Actions']}>
          {filteredHosts.map((host, index) => (
            <tr key={host.id ?? index} className="border-b border-slate-100 last:border-b-0 hover:bg-brand-50/40 transition">
              <td className="px-6 py-4 align-top font-bold text-ink">
                <button
                  type="button"
                  onClick={() => openProfile(host)}
                  className="font-bold text-ink hover:text-brand-600 hover:underline cursor-pointer text-left"
                >
                  {host.name}
                </button>
              </td>
